...
export function MunicipalRecordCard({ record, number }: Props) {
    ...
    <article className="group relative border border-[var(--border)] bg-[var(--paper)] transition-colors hover:border-[var(--ink-faint)] md:grid md:grid-cols-[1fr_180px]">
      <div className="p-6 md:p-8">
        ...
        <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
          <div>
            <dt className="annotation mb-0.5">Entry</dt>
            <dd className="text-[var(--ink-muted)]">
              {record.matchingRequirement ?? "独身証明・面談"}
            </dd>
          </div>
          <div>
        ...
        <div className="flex flex-col gap-3">
          <div className="aspect-[3/4] rounded-sm border border-[var(--border-subtle)] bg-[var(--paper)] p-3">
            ...
            <div className="mt-2 flex flex-col gap-1">
              ...
              <span className="rounded-sm border border-[var(--border-subtle)] px-1 py-0.5 text-[0.5rem]">
                独身証明
              </span>
            </div>
            <div className="mt-3 h-8 rounded-sm bg-[var(--border-subtle)]" />
